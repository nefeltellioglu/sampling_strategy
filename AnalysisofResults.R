...
#setwd("/Users/ntellioglu/OneDrive - The University of Melbourne/Scabies/OneDiseaseDocuments")
#setwd("/Users/ntellioglu/Dropbox/OneDiseaseDocuments")
setwd("/Users/ntellioglu/OneDrive - The University of Melbourne/OneDiseasePaper/Code/Codev2")
...
#parameters-old
{
  ...
  sampling.percentage.list<-c(0.01,0.03,0.05,0.1,0.15,0.2,0.25,0.3,0.4,0.5)
  #pop.size<-600
  #assign scabies
...
#create age & hh distribution graphs
{ 
  #upload new table 
  {
    #setwd("/Users/ntellioglu/Dropbox/OneDiseaseDocuments")
...
#plots
{
  ...
  #hh with different no of cases
  {
    ...
    for (i in c(1:length(prev.method))){
      ...
      hhcolnames<-c("PercHHswithzerocase", 
                    "PercHHswith.atleasthalfcases",
                    "PercHHswithfullcases")
      cols<-c(which(colnames(results1)%in% hhcolnames))
      cols<-cols[order(match(colnames(results1[cols]),hhcolnames))]
      colnames<-factor(c("No infestation",
                         "> Half members",
                         "All members"), 
                       levels=c("No infestation",
                                "> Half members",
                                "All members"))
      results1[cols]<-as.matrix(results1[cols])*100
      df <- results1[cols] %>% 
    ...
    #print(p)
    #dev.off()
    ggsave(filename=paste0("Outputs/HHGroups_Prevalence_All_Methods.pdf"),
           plot=p, width=8, height=4,dpi =300,  units="in", device='pdf')
    ggsave(filename=paste0("Outputs/HHGroups_Prevalence_All_Methods.png"),
           plot=p, width=8, height=4,dpi =300,  units="in", device='png')
  ...
  #version 2-hh with different no of cases
  {
    #overall plot where prevalence is between 20-30%
    prev.method<-unique(results$PrevalenceBasedOn)
    prev.method.names<-c("Random", "High HH-Specific",
    ...
    for (i in c(1:length(prev.method))){
      ...
      results1<-results11[results11$ScabiesPrevalence>=0.2 & results11$ScabiesPrevalence<=0.3,]
      hhcolnames<-c("PercHHswithzerocase", 
                    "PercHHswith.atleastonecase",
                    "PercHHswith.atleasttwocase")
      cols<-c(which(colnames(results1)%in% hhcolnames))
      cols<-cols[order(match(colnames(results1[cols]),hhcolnames))]
      colnames<-factor(c("0",
                         "> 1",
    ...
    #print(p)
    #dev.off()
    ggsave(filename=paste0("Outputs/HHGroups_Prevalence_All_Methodsv2.pdf"),
           plot=p, width=8, height=4,dpi =300,  units="in", device='pdf')
  ...
  #version 3-hh with different no of cases
  {
    #overall plot where prevalence is between 20-30%
    prev.method<-unique(results$PrevalenceBasedOn)
    prev.method.names<-c("Random", "High HH-Specific",
  ...
  #NOT USED sampling prevalence with different strategies -ratios
  {
    bp.vals <- function(x, probs=c(0.05, 0.05,0.5, 0.95, 0.95)) {
    ...
    for (i in c(1:length(prev.method))){
      ...
      plot.list[[i]] <-ggplot(results1, 
                aes(x=as.factor(roundedSamplingPercentage), 
                    y=PrevalenceRatio, color=Sampling.Method)) + 
        ...
        stat_summary(aes(y=PrevalenceRatio),fun=base::mean, 
                     position=position_dodge(.9),
                     geom="point", size=1) + 
        stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), 
                      fun.data=bp.vals, geom="errorbar",size=1)+ theme_minimal() + 
        xlab("Sampling Percentage of the Population") +
        ylab("Sampling Prevalence / Actual Scabies Prevalence") +
      ...
      #ggsave(filename=paste0("Outputs/Prevalence_in_Samplings_in_Method_",prev.method[i],".pdf"),
      #       plot=p, width=8, height=5, units="in")
    }
    ...
    p<-ggplot(results1, 
      ...
      #geom_boxplot(outlier.size =0)+ theme_minimal() + 
      #stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), fun.data=bp.vals, geom="point")+
      stat_summary(aes(y=PrevalenceRatio),fun=median,#base::mean, 
                   position=position_dodge(.9),
                   geom="point", size=1) + 
      stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), 
                    fun.data=bp.vals, geom="errorbar",size=1)+ theme_minimal() + 
    ...
    #print(p)
    #dev.off()
    ggsave(filename=paste0("Outputs/Prevalence_inSamplings_All_Methods.pdf"),
...
#graphs
{
  ...
  #overall plot where prevalence is between 20-30%
  ggplot(results[results$ScabiesPrevalence>=0.2 & results$ScabiesPrevalence<=0.3,], aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3) + theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is between 20-30%")+
    theme(plot.title = element_text(hjust = 0.5))
  
  #overall plot where prevalnece is between 1-10%
  ggplot(results[results$ScabiesPrevalence<=0.1 ,], aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3)+ theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is between 5-10%") +
    theme(plot.title = element_text(hjust = 0.5))
  
  #overall plot where prevalnece is between 10-20%
  ggplot(results[results$ScabiesPrevalence>=0.1 & results$ScabiesPrevalence<=0.2,], aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3) + theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is between 10-20%") +
    theme(plot.title = element_text(hjust = 0.5))
  #overall plot where prevalnece is between 30-40%
  ggplot(results[results$ScabiesPrevalence>=0.3 & results$ScabiesPrevalence<=0.4,], aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3) + theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is between 30-40%") +
    theme(plot.title = element_text(hjust = 0.5))
  
  #based on pop size
  popsize<-c(500,1000)
  prev.limits<-c(0.2,0.3)
  #prev.limits<-c(0.1,0.4)
  ggplot(results[results$PopulationSize %in% popsize & 
                   results$ScabiesPrevalence>=prev.limits[1] & 
                   results$ScabiesPrevalence<=prev.limits[2],], 
         aes(x=as.factor(roundedSamplingPercentage), 
             y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3)+ theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle(paste0("Actual Scabies Prevalence in ", prev.limits[1]*100,"-", prev.limits[2]*100,"% & Population Size in ", popsize[1],"-", popsize[2] , collapse = "")) +
    theme(plot.title = element_text(hjust = 0.5))+ylim(0,6) 
  
  popsize<-c(1500,2000,2500)
  #popsize<-c(3000,3500,4000)
  ggplot(results[results$PopulationSize %in% popsize & 
                   results$ScabiesPrevalence>=prev.limits[1] & 
                   results$ScabiesPrevalence<=prev.limits[2],], 
         aes(x=as.factor(roundedSamplingPercentage), 
    ...
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle(paste0("Actual Scabies Prevalence in ", prev.limits[1]*100,"-", prev.limits[2]*100,"% & Population Size in ", popsize[1],"-", popsize[2],"-",popsize[3] , collapse = "")) +
    theme(plot.title = element_text(hjust = 0.5))+ylim(0,6) 
  
  #show the bias in school sampling
  results1<-results[results$ScabiesPrevalence==0.3,]
  results1<-results1[results1$PrevalenceBasedOn!="age" &
                       results1$PrevalenceBasedOn!="hh-age",]
  #View(results1[results1$roundedSamplingPercentage==0.3,])
  ggplot(results1, aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    geom_boxplot(outlier.size =0.3) + theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is 30% and not age-dependent")+
    theme(plot.title = element_text(hjust = 0.5))
  ...
  ggplot(results1, aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    ...
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is 30%")+
    theme(plot.title = element_text(hjust = 0.5))
...
#Additional Graph and Table results_18Feb2021
{
  #upload new table 
  {
    #setwd("/Users/ntellioglu/Dropbox/OneDiseaseDocuments")
    ...
    allhhsizes<-hh.dist$NoPeople
    allhhsizes<-all.sizes2
    rounding.list=c(0.01,0.03,0.05,0.1,0.15,0.2,0.25,0.3,0.4,0.5,0.6,0.7,0.8)
    rounded.perc<-unlist(sapply(1:nrow(results), 
                                function(x) rounding.list[which.min(abs(rounding.list - results$ActualSamplingPercentage[x] ))]
    ))
    #head(rounded.perc,10)
  ...
  #Compare hh and age histograms
  {
    #hh size histograms
    {
      ...
      colnames<-factor(paste0(all.sizes2), 
                       levels=paste0(all.sizes2))
      results1[49]<-rowSums(results1[49:61],na.rm=TRUE)
      results1[cols]<-as.matrix(results1[cols])
      mydf1 <- data.frame(group=colnames,
                          mean=colMeans(results1[cols],na.rm =TRUE ), 
                          sd=apply(results1[cols], 2, na.rm =TRUE,sd))
      mydf1
      mytable2<-data.frame(NoPeople=c(1:6), perc=mydf1$mean[1:6], sd=mydf1$sd[1:6], type="Simulation")
      mytable2$NoPeople[6]<-"6+"
      mytable<-rbind.fill(mytable,mytable2)
      
      p<-ggplot(mytable, aes(x=NoPeople,y=perc*100,fill=type)) + 
        geom_bar(stat="identity", position=position_dodge()) +
        geom_errorbar(aes(ymin=perc*100-sd*100, ymax=perc*100+sd*100), width=.2,
                      position=position_dodge(.9))+ theme_minimal() + 
      ...
      ggsave(filename=paste0("Outputs/HHSizeDistribution.png"),
             plot=p, width=5, height=2.5,dpi =130,  units="in", device='png')
    }
    #age histograms
    {
     ...
     p<- ggplot(mytable, aes(x=groups,y=perc*100,fill=type)) + 
        ...
        theme(plot.title = element_text(hjust = 0.5))+ 
        guides(fill=guide_legend(title="Datasets"))
      ggsave(filename=paste0("Outputs/AgeDistribution.png"),
             plot=p, width=5, height=2.5,dpi =130,  units="in", device='png')
    }
  ...
  #plots
  {
    #age groups
    {
      ...
      sum.results
      p <- ggplot(sum.results, aes(x=group,y=mean,fill=basedon)) + 
        geom_bar(stat="identity", position=position_dodge()) +
        geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                      position=position_dodge(.9))+ theme_minimal() + 
        xlab("") +
    ...
    #hh with different no of cases
    {
      ...
      for (i in c(1:length(prev.method))){
        ...
        results1<-results11[results11$ScabiesPrevalence>=0.2 & results11$ScabiesPrevalence<=0.3,]
        hhcolnames<-c("PercHHswithzerocase", 
                      "PercHHswithhalfcases",
                      "PercHHswithfullcases")
        cols<-c(which(colnames(results1)%in% hhcolnames))
        cols<-cols[order(match(colnames(results1[cols]),hhcolnames))]
        colnames<-factor(c("No infection",
                           "Half",
                           "All"), 
    ...
    #sampling prevalence with different strategies
    {
      bp.vals <- function(x, probs=c(0.05, 0.05,0.5, 0.95, 0.95)) {
        r <- quantile(x, probs=probs , na.rm=TRUE)
        #r = c(r[1:2], exp(mean(log(x))), r[3:4])
    ...
    `%notin%` <- Negate(`%in%`)
    {
      ...
      for (i in c(1:length(prev.method))){
        ...
        mycolnames<-c("PrevalenceinpreschoolSample",
                      ...
                      "PrevalenceinSample")
        results1$PrevalenceinSample<-results1$PrevalenceinSample*100
        results1<-results1[results1$roundedSamplingPercentage %notin% c(0.01,0.03,0.6),]
        p<-ggplot(results1, 
                  aes(x=as.factor(roundedSamplingPercentage*100), 
                      y=PrevalenceinSample, color=Sampling.Method)) + 
          #geom_boxplot(outlier.size =0)+ theme_minimal() + 
          ...
          stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), 
                        fun.data=bp.vals, geom="errorbar",size=1)+ theme_minimal() + 
          xlab("Sampling % of the Population") +
          ylab("Observed Prevalence %") +
          ggtitle(paste0("Method: ", prev.method.names[i],collapse = "")) +
      ...
      ggsave(filename=paste0("Outputs/Actual_Prevalence_inSamplings_All_Methods.png"),
             plot=p, width=6, height=3,dpi =150,  units="in", device='png')
    ...
    #sampling prevalence with different strategies
    {
      ...
      p<-ggplot(results1, 
        ...
        stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), 
                      fun.data=bp.vals, geom="errorbar",size=1)+ theme_minimal() + 
        xlab("Scabies Assignment Approaches") +
        ylab("Observed Prevalence %") +
        #ggtitle(paste0("Actual Scabies Prevalence 20-30% ",collapse = "")) +
        theme(plot.title = element_text(hjust = 0.5))+#ylim(0,6) +
        geom_hline(yintercept=20,linetype="dashed",  color = "red")+
  ...
  #tables
  {
    ...
    for (i in c(1:length(prev.method))){
      ...
      given.method.list<- c("Random Sampling","Household Sampling")  #unique(results$Sampling.Method)
      given.popsizes.list<-as.list(unique(results$PopulationSize))
      given.popsizes.list[[9]]<-c(500,1000,1500)
      given.popsizes.list[[10]]<-c(2000,2500)
      given.popsizes.list[[11]]<-c(3000,3500,4000)
      ...
      precision.levels<-c(0.01,0.02,0.05,0.1)
      library(gridExtra)
      results$diffPrevalence<-results$PrevalenceinSample-results$ScabiesPrevalence
      
      for(ci in precision.levels){
        for (given.method in given.method.list){
          mytable<-data.frame(matrix(ncol = 3, nrow = 4))
          colnames(mytable)<-c("[500-1500]", "(1500-2500]", "(2500-4000]")
          rownames(mytable)<-c("[0.05-0.1]", "(0.1-0.2]","(0.2-0.3]","(0.3-0.4]")
          for (r in c(1:4)){
            given.actual.prevalence<- given.actual.prevalence.list[12:15][[r]]
            for (c in c(1:3)){
              given.popsizes<-given.popsizes.list[9:11][[c]]
              prev.limits<-c(min(given.actual.prevalence),max(given.actual.prevalence) )
              if (is.na(prev.limits[2])){prev.limits[2]<-prev.limits[1]}
              results2<-results[results$ScabiesPrevalence>=prev.limits[1] &
                                  results$ScabiesPrevalence<=prev.limits[2] &
                                  results$PopulationSize %in% given.popsizes, ]
              results2<-results2[results2$roundedSamplingPercentage!=0.6,]
              df3<-results2 %>%
                group_by(Sampling.Method, roundedSamplingPercentage) %>%
                summarize_at(vars(diffPrevalence), funs(!!!p_funs))
              df4<-results2 %>%
              ...
              if (sum(upper.diff<0)==0 | sum(lower.diff<0)==0){
                #none of the observed CI is in the range of (-ci,ci)
                cur.sampling.percentage<-min(max(my.df$Sampling.Percentage)*100,50)
                mytable[r,c]<-paste0(cur.sampling.percentage,"+", collapse="")
              }else{
                #there is at least one CI in the range of (-ci,ci)
                #return the CI correspond to the smaller sampling percentage
                my.index<-max(min(which(upper.diff<=0)),min(which(lower.diff<=0)))
                cur.sampling.percentage<-my.df$Sampling.Percentage[my.index]
                mytable[r,c]<-cur.sampling.percentage*100
              }
            }
          }
          print(mytable)
          filename<-paste0("Outputs/",(p[3]-p[1])*100,"%CI_SummaryTables","_",prev.method[i],"_",given.method,"_Accuracy_",ci, ".csv")
          write.csv(mytable,filename)
        }
      }
...
#new table results_24Jan2021_v1(actual)
{
      ...
      for (r in c(1:4)){
        ...
        for (c in c(1:3)){
          ...
          if (sum(upper.diff<0)==0 | sum(lower.diff<0)==0){
            ...
            #return the CI correspond to the smaller sampling percentage
            my.index<-max(min(which(upper.diff<=0)),min(which(lower.diff<=0)))
            cur.sampling.percentage<-my.df$Sampling.Percentage[(my.index-1)*noofdifferentprevalences+1]
            mytable[r,c]<-cur.sampling.percentage*100
          }
        }
      }
      print(mytable)
      filename<-paste0("Outputs/",(p[3]-p[1])*100,"%CI_SummaryTables","_",given.method,"_Accuracy_",ci, ".csv")
      write.csv(mytable,filename)
    }
  }
...
#new table results_24Jan2021_v0
{
  ...
  #For ex, 0.02 precision correspond to 18-22% range around 20% actual scabies prevalence.
  #Then,
  #create a 90% CI (or 95% CI) of observed prevalences given a pop size, sampling method, and actual scabies prevalence range.
  #Then,
  #check for all the sampling percentages if the observed prevalence CI (of all the actual prevalence values in the actual prevalence range)
  #is in the given precision range. Report the smallest sampling percentage as 
  #the min sampling necessary to obtain a prevalence in the given range with 90% CI
  p <- c(0.025, 0.5, 0.975)
  p_names <- map_chr(p, ~paste0(.x*100))
  ...
  #CI.values<-c(0.2,0.3)
  precision.levels<-c(0.01,0.02,0.05,0.1)
  tolerance<-0.0 #allows CI 0.45 instead of 0.4 when 20% is asked 
  library(gridExtra)
  ...
  for(ci in precision.levels){
    for (given.method in given.method.list){
      ...
      for (r in c(1:4)){
        ...
        for (c in c(1:3)){
          ...
          results2<-results[results$ScabiesPrevalence>=prev.limits[1] &
                              ...
                              results$PopulationSize %in% given.popsizes, ]
          df3<-results2 %>%
            group_by(Sampling.Method, roundedSamplingPercentage,ScabiesPrevalence) %>%
            summarize_at(vars(PrevalenceinSample), funs(!!!p_funs))
          df4<-results2 %>%
            group_by(Sampling.Method, roundedSamplingPercentage,ScabiesPrevalence) %>%
            dplyr::summarize(n=dplyr::n())
          df3$n<-df4$n
          my.df<-df3[df3$Sampling.Method==given.method,]
          colnames(my.df)<-c("Sampling.Method", "Sampling.Percentage","ScabiesPrevalence",
                             paste0("ObservedPrevalence_Quantile",p[1]),
                             paste0("ObservedPrevalence_Quantile",p[2]),
                             paste0("ObservedPrevalence_Quantile",p[3]), "Number of Runs")
          
          #CI<-my.df$`ObservedPrevalence_Quantile0.95`-my.df$`ObservedPrevalence_Quantile0.05`
          CI<-unlist(unname(my.df[,6]-my.df[,4]))
          diff.from.desired<-(CI-2*ci)
          noofdifferentprevalences<-length(unique(my.df$ScabiesPrevalence))
          
          if (sum(tail(diff.from.desired,n=noofdifferentprevalences)<0)<noofdifferentprevalences){
            cur.sampling.percentage<-max(my.df$Sampling.Percentage)*100
            mytable[r,c]<-paste0(cur.sampling.percentage,"+", collapse="")
          }else{
            diff.from.desired<-CI-2*ci-tolerance
            #separate diff.from.desired into chunks
            splitted<-split(diff.from.desired, ceiling(seq_along(diff.from.desired)/noofdifferentprevalences))
            #return values in the CI range
            in.CI.list<-unlist(lapply(1:length(splitted), function(x) sum((splitted[[x]]<0)*1)==length(splitted[[x]])))*1
            #return smallest  sampling percentage
            my.index<-min(which(in.CI.list==1))
            cur.sampling.percentage<-my.df$Sampling.Percentage[(my.index-1)*noofdifferentprevalences+1]
            mytable[r,c]<-cur.sampling.percentage*100
          }
        }
      ...
      filename<-paste0("Outputs/",(p[3]-p[1])*100,"%CI_SummaryTables","_",given.method,"_Accuracy_",ci, ".csv")
      write.csv(mytable,filename)
      # filename<-paste0("Outputs/SummaryTables","_",given.method,"_Accuracy_",ci, ".pdf")
      # g1 <- tableGrob(mytable, rows=rownames(mytable))
      # 
      # pdf(file = filename)
      # grid.table(g1)
      # dev.off()
  ...
  for(ci in CI.values){
    for (given.method in given.method.list){
      ...
      for (r in c(1:4)){
        given.actual.prevalence<- given.actual.prevalence.list[12:15][[r]]
        for (c in c(1:3)){
          #where prevalence is between 
          #ci<-CI.values[1]
          #given.method<-given.method.list[[1]]
          #given.actual.prevalence<-given.actual.prevalence.list[12:15][[1]]
          #given.popsizes <- given.popsizes.list[9:11][[1]]
          ...
          df3<-results2 %>% 
            group_by(Sampling.Method, roundedSamplingPercentage) %>% 
            summarize_at(vars(PrevalenceRatio), funs(!!!p_funs))
          ...
          df3$n<-df4$n
          #View(df3)
          df3<-df3[df3$roundedSamplingPercentage<=0.5,]
          
          
          my.df<-df3[df3$Sampling.Method==given.method,]
          colnames(my.df)<-c("Sampling.Method", "Sampling.Percentage",
                             "Observed/ActualPrevalence_Quantile0.05",
                             "Observed/ActualPrevalence_Quantile0.50",
                             "Observed/ActualPrevalence_Quantile0.95", "Number of Runs")
          CI<-my.df$`Observed/ActualPrevalence_Quantile0.95`-my.df$`Observed/ActualPrevalence_Quantile0.05`
          #print(CI)
          #print(which.min(abs(CI-2*ci)))
          diff.from.desired<-abs(CI-2*ci)
          if (sum(CI>2*ci)==length(CI)){
            cur.sampling.percentage<-max(my.df$Sampling.Percentage)*100
            mytable[r,c]<-paste0(cur.sampling.percentage,"+", collapse="")
          }else{
            diff.from.desired<-CI-2*ci-tolerance
            #print(which(diff.from.desired<0)[1])
            cur.sampling.percentage<-my.df$Sampling.Percentage[which(diff.from.desired<0)[1]]
            mytable[r,c]<-cur.sampling.percentage*100
          }
        }
      }
      print(mytable)
      filename<-paste0("Outputs/SummaryTables","_",given.method,"_Accuracy_",ci, ".csv")
      write.csv(mytable,filename)
      # filename<-paste0("Outputs/SummaryTables","_",given.method,"_Accuracy_",ci, ".pdf")
      # g1 <- tableGrob(mytable, rows=rownames(mytable))
      # 
...
#old table results
{
  ...
  given.method.list<-unique(results$Sampling.Method)
  given.popsizes.list<-as.list(unique(results$PopulationSize))
  given.popsizes.list[[9]]<-c(500,1000)
  given.popsizes.list[[10]]<-c(1500,2000,2500)
  given.popsizes.list[[11]]<-c(3000,3500,4000)
  ...
  for (given.method in given.method.list){
    for (given.actual.prevalence in given.actual.prevalence.list ){
      for (given.popsizes in given.popsizes.list){
        #where prevalence is between 
        prev.limits<-given.actual.prevalence
        if (is.na(prev.limits[2])){prev.limits[2]<-prev.limits[1]}
        results2<-results[results$ScabiesPrevalence>=prev.limits[1] &
                            results$ScabiesPrevalence<=prev.limits[2] &
                            results$PopulationSize %in% given.popsizes, ]
        ...
        my.df<-df3[df3$Sampling.Method==given.method,]
        colnames(my.df)<-c("Sampling.Method", "Sampling.Percentage",
                           "Observed/ActualPrevalence_Quantile0.025",
                           "Observed/ActualPrevalence_Quantile0.50",
                           "Observed/ActualPrevalence_Quantile0.975", "Number of Runs")
        filename<-paste0("Outputs/Pop",paste(given.popsizes,collapse = "_"),"_",given.method,"_Prevalence_",
                         paste(given.actual.prevalence,collapse = "_"), ".csv")
        write.csv(my.df,filename)
      }
    }
  }
  ...
  #View(df[ df$ScabiesPrevalence==0.2 & df$roundedSamplingPercentage == 0.1 ,])
...
#new graphs
{
  ...
  #overall plot where prevalence is between 20-30%
  ggplot(results[results$ScabiesPrevalence>=0.2 & results$ScabiesPrevalence<=0.3,], aes(x=as.factor(roundedSamplingPercentage), y=PrevalenceRatio, fill=Sampling.Method)) + 
    stat_summary( aes(fill=Sampling.Method), position=position_dodge(.9), fun.data=bp.vals, geom="boxplot")+ theme_minimal() + 
    xlab("Sampling Percentage of the Population") +
    ylab("Sampling Prevalence / Actual Scabies Prevalence") +
    ggtitle("Actual Scabies Prevalence is between 20-30%")+
    theme(plot.title = element_text(hjust = 0.5))+
